...
Companion to [mvp-plan.md](./mvp-plan.md). That document covers the near-term "spinning
...
| Stage | Goal | Status |
|---|---|---|
| **MVP0 — Engine Foundation** | Single-player render loop: ECS, Vulkan + WebGPU rendering, fixed-timestep loop, `scene.json` loading | ✅ Done — this is `mvp-plan.md`'s entire scope |
| **MVP1a — Playable Prototype** | One controllable character moving around one hand-built world with a simple NavMesh-driven AI to chase/avoid, using the *existing* renderer as-is (no skinning, no render-graph rewrite) | 🚧 In progress — kinematic movement + third-person follow camera done (2026-07-11); visible ground plane done (2026-07-11, Vulkan+WebGPU). **Navigation regressed (2026-08-30):** the `recast4j` backend left with `samples:scene3d-playground` and only the `NavMesh` interface remains, so chase AI has no path source on any target. Dodge/avoid behavior still open |
...
| Item | Status | Priority | Stage | Approach |
|---|---|---|---|---|
| NavMesh Generation & Dynamic Obstacle Avoidance | 🔲 Reverted to not started (2026-08-30) | P0 | MVP1a | The `recast4j` implementation (`DemoNavMesh`, `DemoNavMeshGeometry`) moved into `samples:scene3d-playground` on 2026-08-05 and was deleted with that sample. Only the `NavMesh` interface in `:awake:scene:scene-core` survives; the `recast4j` version catalog entries were dead and are removed. Recast was JVM-only, so it never covered iOS/wasmJs. Replacement: a heightmap-derived navgrid in `commonMain`, covering all four targets rather than Recast's two -- see [the navgrid plan](tasks/2026-08-30-navgrid-navigation-plan.md). |
| ECS-Coupled Behavior Trees | 🚧 Partial (2026-07-11) | P0 | MVP1a | First real behavior via `ChaseAiSystem`, now engine-owned in `:awake:scene:scene-core` -- one fixed chase behavior (periodic repath + kinematic waypoint steering), not a general branching behavior-tree framework, and still one system instance per NPC rather than an ECS query. Plan for the general case: [behavior tree & state machine](tasks/2026-08-30-behavior-tree-state-machine-plan.md). |
...
- This roadmap is deliberately not broken into `- [ ]` checkboxes like `mvp-plan.md`'s
