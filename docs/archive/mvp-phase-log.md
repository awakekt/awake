# Awake Engine — MVP phase log (archive)

Historical record of how phases 0–8 were executed, kept verbatim. Superseded as a planning
document by [../mvp-plan.md](../mvp-plan.md), which holds the live roadmap; decisions moved to
[../reference/decision-log.md](../reference/decision-log.md).

Read this to answer "why was it built this way" or "what did we already try". Statuses and dates
below are as-of their entry and are not maintained.
...
> [mmorpg-roadmap.md](../mmorpg-roadmap.md).
...
- [x] Upgrade Kotlin to the latest stable bug-fix release, 2.4.10 *(2026-08-21)*; retain
      AGP 9.2.1, Compose Multiplatform 1.11.1, Gradle 9.6.1, and Dokka 2.0.0.
...
hierarchy propagation, and query iteration at both scales; Fleks leads component add/remove
at both scales and family churn at 10k, while Artemis-odb leads family churn at 100k).
...
- [x] **Direct array access**: `Family1.components()`/`componentAt(index)` and
      ...
      access instead of a per-entity callback. `EcsTag` columns stay payload-free during
      iteration and lazily materialize the compatible repeated array only when requested.
- [x] **Payload-free tags and family columns (2026-08-21)**: singleton `EcsTag` values store
      entity membership plus one canonical reference in both `ComponentStore` and maintained
      families. The family implementation is split into focused cache/value-column files;
      decision-grade JMH improved tag-family iteration by roughly 2.4× while ordinary-family
      iteration remained within the 5% regression gate. See
      [the implementation report](../tasks/archive/2026-08-21-ecs-family-tag-columns.md).
...
- [x] **Benchmark harness** (`:awake:ecs:benchmark`, JVM-only): real, same-JVM comparison
      ...
      value-class boxing removal in `TransformSystem`, and cached type-ID mutation that removed
      repeated class resolution from structural loops.
- [ ] **Adaptive bulk structural mutation (planned 2026-08-21)**: retain immediate
      `World.add`/`remove`, prototype an allocation-free opt-in command buffer, and choose
      incremental family maintenance versus one dirty-family rebuild from measured crossover data.
      The public API ships only if it beats fresh same-semantics Fleks 10k and Artemis 100k churn
      controls without more than 5% regression elsewhere. See
      [the active plan](../tasks/2026-08-21-ecs-adaptive-bulk-mutation-plan.md).
