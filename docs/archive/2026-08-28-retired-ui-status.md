# Archived: UI status report

This document is retained for history only. It describes the deleted immediate-mode UI module
stack and must not be used as the current support matrix.
...
`skills/awake-shadcn-recipe-authoring/SKILL.md` (how to read Tailwind, source-of-truth rule).
...
|---|---|---|---|---|
| Spacing scale (`Tw.Spacing`) | ✅ | ✅ | ✅ | Generated from a vendored Tailwind scale by `:awake:tailwind-generator`. |
...
1. **Assert against the pinned reference, not against ourselves.** `third_party/shadcn-ui-ref`
   holds the real `.tsx`; `tools/shadcn/extract_shadcn_tokens.py` already parses it. A test that extracts
