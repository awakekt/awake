...
| State container & property delegation | `mutableStateOf(value)`, `State<T>`, `MutableState<T>` | Same, with `by` property delegates and destructuring | **Parity** | Canonical state container for `remember`. Writing updates immediately in the single-pass loop without needing compiler restart groups |
