...
| `awake-game-runtime-engineer`     | Active | `:awake:engine:platform`, `:awake:engine:bootstrap`, optional Compose app integration, scene-session adoption, `:samples:studio`, `:samples:ui-showcase`, MVI state flow | `balanced-coding` | `claude-sonnet-5` |
