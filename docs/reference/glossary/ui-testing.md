...
| **frame** | One moment of UI input and drawing. | One call to `composeFrame` or `ComposeTestSession.frame`. |
...
- One static frame: use `composeFrame(...)`.
- Several frames or user input: use `composeTestSession(...)`.
