...
- **WebGPU shadows now render (2026-08-24).** The remaining half named below — a WGSL variant
  carrying the `shadowMap`/`shadowMapSampler` bindings plus `LitShadowUniformLayout`'s floats
  through `primaryDraw` — landed. One shared `lit_shadow` serves both backends by declaring
  the map `texture_depth_2d`, `PreparedDraw` gained an optional `shadowBinding` bound at the
  set the joint palette also uses (mutually exclusive by pipeline), and `WebGpuCapabilities`
  stopped narrowing the pass away. Original entry follows.
