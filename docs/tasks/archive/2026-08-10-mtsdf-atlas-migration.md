...
Status: **implemented**. The generated Roboto faces use committed MTSDF atlases, the default
weighted font routes through the distance-field shader, and raster fidelity gates cover the
headless path plus the pinned Chromium bounds.
...
Not "it compiles". The current measurements are:

- `FontReferenceFidelityTest` compares 12/14/16px `roundvsflat` ink bounds against the pinned
  Chromium captures.
- `FontRasterFidelityTest` checks monotonic raster growth, weighted-face baseline alignment, and
  non-binary MTSDF edge coverage.
- `RendererHeadlessUiGlyphBaselineTest.headlessMtsdfGlyphCoverageGrowsAcrossUiSizes` verifies the
  trueSans MTSDF shader on headless Vulkan/MoltenVK, including scale growth and partial edge coverage.

## Related follow-ups

- **Future tightening.** The current gate intentionally compares geometry and coverage invariants,
  not raw pixels, because Chromium and Awake use different rasterizers. A normalized pixel oracle
  can be added once both paths expose the same color-space and sampling contract.
