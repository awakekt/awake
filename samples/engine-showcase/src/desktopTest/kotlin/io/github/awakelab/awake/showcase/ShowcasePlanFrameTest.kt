...
import io.github.awakelab.awake.engine.bootstrap.dsl.appSpec
...
import io.github.awakelab.awake.vulkan.renderer.readPresentedPixels
...
/**
 * The showcase's own render plan, rendered without a window.
 *
 * Every shadow probe in the engine builds its own renderer: one pipeline, one depth target, no
 * content features. That is a fixture, not this app -- and a fixture cannot show a scene that
 * looks wrong on screen while every probe passes, which is exactly what happened to the
 * heightfield showcase's terrain. This boots `VulkanEngine` on
 * [EngineShowcaseRenderPlan] itself -- its skybox, its depth fog, its scene-depth pass, its
 * shadow pre-pass -- and renders the real [TerrainExampleAsset] through it.
 *
 * The terrain's own vertex colours are green, and its normals point up, so under this scene's
 * light it must come out green. Ambient alone would be about a fifth as bright: the lit shader's
 * ambient is 0.08 of base colour, which for this terrain tone-maps and gamma-encodes to roughly
 * 43 -- so a terrain that is fully shadowed, or lit by nothing, fails here rather than merely
 * looking dim in a screenshot.
 *
 * Measured green about 125 through BOTH recordings -- `renderToTexture` and `Renderer.draw` --
 * so the two agree and neither is what darkens the terrain on screen. What is still not covered
 * is the app's scene wiring: these tests hand the renderer a camera, a light and one draw call,
 * where the running app builds all three from ECS components through `RenderSystem`.
 */
