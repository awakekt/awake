 ...
 * same source drawings into an RGB distance atlas, leaving room for a future true MSDF asset
 * pipeline without changing the shared UI contracts again.
 *
 * **Built but not wired.** Nothing calls [UiFonts.msdf] yet -- text renders through the embedded
 * fixed-grid bitmap font, which is the standing decision until text quality demands otherwise. So
 * this path is also untested: no caller means no coverage, and the numbers below have never been
 * checked against a rendered glyph.
 *
 * That is worth stating because the previous version of this comment read in the present tense --
 * "so Vulkan, WebGPU, previews, and snapshot tests can all exercise the same distance-field text
 * path now" -- which describes a wiring that does not exist. Unused code that claims to be in use
 * reads as dead on the next audit; this one was deleted on exactly that reading and restored.
 */
class MsdfFont(
