...
class QuatOpsTest {
    ...
    fun rotatingAPointMatchesTheEquivalentMatrix() {
        val rotation = Quat.fromAxisAngle(Vec3f(0f, 1f, 0f), QUARTER)
        val point = Vec3f(1f, 0f, 0f)
    ...
    fun composingTwoRotationsAgreesWithComposingTheirMatrices() {
        val first = Quat.fromAxisAngle(Vec3f(0f, 1f, 0f), 0.7f)
        val second = Quat.fromAxisAngle(Vec3f(1f, 0f, 0f), -0.3f)
    ...
    fun aRotationFollowedByItsInverseIsANoOp() {
        val rotation = Quat.fromAxisAngle(Vec3f(0.3f, 1f, -0.2f), 1.1f)
        val undone = rotation * assertNotNull(rotation.inverse())
        val point = Vec3f(2f, -1f, 0.5f)
    ...
    fun eulerAnglesRoundTrip() {
        val euler = Vec3f(0.4f, -0.9f, 0.25f)
    ...
    fun fromEulerMatchesTheTransformMatrixOrder() {
        val euler = Vec3f(0.3f, 0.8f, -0.5f)
    ...
    fun gimbalLockStillDescribesTheSameRotation() {
        val locked = Quat.fromEuler(Vec3f(QUARTER, 0.6f, 0f))
