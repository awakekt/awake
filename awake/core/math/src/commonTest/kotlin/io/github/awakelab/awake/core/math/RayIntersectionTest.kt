...
class RayIntersectionTest {
    ...
    fun aDirectionIsNormalizedSoDistancesAreInWorldUnits() {
        val ray = Ray(Vec3f(0f, 0f, 0f), Vec3f(0f, 0f, -5f))
        assertEquals(1f, ray.direction.length3(), TOLERANCE)
        val hit = assertNotNull(ray.intersectSphere(Vec3f(0f, 0f, -10f), radius = 1f))
        assertEquals(9f, hit, TOLERANCE, "an unnormalized direction must not scale the distance")
    }

    @Test
    fun aRayPointingAwayFromASphereMisses() {
        val ray = Ray(Vec3f(0f, 0f, 0f), Vec3f(0f, 0f, 1f))
        assertNull(ray.intersectSphere(Vec3f(0f, 0f, -10f), radius = 1f))
    }
    ...
    fun aRayStartingInsideASphereHitsImmediately() {
        val ray = Ray(Vec3f(0f, 0f, 0f), Vec3f(1f, 0f, 0f))
        assertEquals(0f, assertNotNull(ray.intersectSphere(Vec3f(0f, 0f, 0f), radius = 2f)))
    }

    @Test
    fun aRayHitsTheNearFaceOfABox() {
        val box = Aabb(Vec3f(-1f, -1f, -1f), Vec3f(1f, 1f, 1f))
        val ray = Ray(Vec3f(0f, 0f, 5f), Vec3f(0f, 0f, -1f))
    ...
    fun anAxisParallelRayIsHandledWithoutSpecialCasing() {
        val box = Aabb(Vec3f(-1f, -1f, -1f), Vec3f(1f, 1f, 1f))
        assertNotNull(Ray(Vec3f(-5f, 0f, 0f), Vec3f(1f, 0f, 0f)).intersectAabb(box), "should hit")
        assertNull(
            Ray(Vec3f(-5f, 9f, 0f), Vec3f(1f, 0f, 0f)).intersectAabb(box),
            "should miss above",
        )
    }
    ...
    fun aRayHitsTheGroundPlaneAtTheExpectedDistance() {
        val ground = Plane.through(Vec3f(0f, 0f, 0f), Vec3f(0f, 1f, 0f))
        val ray = Ray(Vec3f(0f, 3f, 0f), Vec3f(0f, -1f, 0f))
    ...
    fun aRayParallelToAPlaneMissesIt() {
        val ground = Plane.through(Vec3f(0f, 0f, 0f), Vec3f(0f, 1f, 0f))
        assertNull(Ray(Vec3f(0f, 3f, 0f), Vec3f(1f, 0f, 0f)).intersectPlane(ground))
    }

    @Test
    fun aPlaneReportsWhichSideAPointIsOn() {
        val ground = Plane.through(Vec3f(0f, 0f, 0f), Vec3f(0f, 1f, 0f))
        assertTrue(ground.signedDistanceTo(Vec3f(0f, 2f, 0f)) > 0f)
        assertTrue(ground.signedDistanceTo(Vec3f(0f, -2f, 0f)) < 0f)
        val projected = ground.project(Vec3f(1f, 5f, -2f))
