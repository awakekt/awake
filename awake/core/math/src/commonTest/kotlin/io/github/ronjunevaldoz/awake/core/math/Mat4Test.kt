...
class Mat4Test {
    ...
    fun rotationsPreserveLength() {
        ...
        val rotated =
            Mat4().rotateX(0.4f).rotateY(-1.1f).rotateZ(2.3f).applyToColumnVector(v.x, v.y, v.z)
    ...
    fun rotateAboutAnArbitraryAxisNormalisesThatAxis() {
        val unit = Mat4().rotate(0.9f, Vec3f(0f, 0f, 1f))
        val scaled = Mat4().rotate(0.9f, Vec3f(0f, 0f, 25f))
    ...
    fun rotateAboutZeroAngleIsTheIdentity() {
        assertMat4(Mat4(), Mat4().rotate(0f, Vec3f(1f, 2f, 3f)))
    }

    /**
     * The convention this repo documents on [Mat4.times] and [Lens.viewProjectionMatrix]:
    ...
    fun fromTrsWithIdentityRotationAndUnitScaleIsAPureTranslation() {
        val trs = Mat4.fromTrs(Vec3f(1f, 2f, 3f), Quat.IDENTITY, Vec3f(1f, 1f, 1f))
    ...
    fun rotateAboutAnAxisShouldMatchTheDedicatedPerAxisHelper() {
        assertMat4(Mat4().rotateX(0.7f), Mat4().rotate(0.7f, Vec3f(1f, 0f, 0f)))
        assertMat4(Mat4().rotateY(0.7f), Mat4().rotate(0.7f, Vec3f(0f, 1f, 0f)))
        assertMat4(Mat4().rotateZ(0.7f), Mat4().rotate(0.7f, Vec3f(0f, 0f, 1f)))
    }

    /** Regression: plus ran [times]'s triple loop with '*' swapped for '+', so every entry of
     * `identity + identity` came out 2 instead of the diagonal being 2 and the rest 0. */
    @Test
    fun plusLeavesOffDiagonalEntriesAloneWhenAddingTwoIdentities() {
        val sum = Mat4() + Mat4()

        val expected = IDENTITY_DATA.map { it * 2f }
        assertEquals(expected, sum.data.toList())
    }
