...
class FrustumTest {
    ...
    fun cornersMatchHandComputedPositionsForIdentityViewCamera() {
        ...
        assertVec3Equals(Vec3f(-1f, -1f, -1f), corners[0]) // near bottom-left
        assertVec3Equals(Vec3f(1f, -1f, -1f), corners[1]) // near bottom-right
        assertVec3Equals(Vec3f(1f, 1f, -1f), corners[2]) // near top-right
        assertVec3Equals(Vec3f(-1f, 1f, -1f), corners[3]) // near top-left
        assertVec3Equals(Vec3f(-10f, -10f, -10f), corners[4]) // far bottom-left
        assertVec3Equals(Vec3f(10f, -10f, -10f), corners[5]) // far bottom-right
        assertVec3Equals(Vec3f(10f, 10f, -10f), corners[6]) // far top-right
        assertVec3Equals(Vec3f(-10f, 10f, -10f), corners[7]) // far top-left
    }
    ...
    fun cornersFollowTheCameraBasisWhenItLooksDownPlusX() {
        ...
        assertVec3Equals(Vec3f(1f, 1f, 1f), corners[2]) // near top-right
        assertVec3Equals(Vec3f(1f, 1f, -1f), corners[3]) // near top-left
        assertVec3Equals(Vec3f(10f, -10f, -10f), corners[4])
        assertVec3Equals(Vec3f(10f, 10f, 10f), corners[6])
    }

    @Test
    fun movingTheCameraTranslatesEveryCorner() {
        val atOrigin = Frustum.corners(identityViewCamera(), aspect = 1f)
        val offset = Vec3f(3f, -4f, 5f)
        val moved = Frustum.corners(
            Lens(
                eye = offset,
                center = offset + Vec3f(0f, 0f, -1f),
                up = Vec3f(0f, 1f, 0f),
                fovYRadians = (kotlin.math.PI / 2.0).toFloat(),
                near = 1f,
                far = 10f,
    ...
    fun boxOnTheForwardAxisIntersects() {
        val camera = identityViewCamera()
        val box = Aabb(Vec3f(-0.5f, -0.5f, -5.5f), Vec3f(0.5f, 0.5f, -4.5f))
    ...
    fun boxBehindTheCameraDoesNotIntersect() {
        val camera = identityViewCamera()
        // Lens looks down -Z from the origin -- +Z is directly behind it.
        val box = Aabb(Vec3f(-0.5f, -0.5f, 4.5f), Vec3f(0.5f, 0.5f, 5.5f))
    ...
    fun boxAboveTheTopPlaneDoesNotIntersect() {
        ...
        val box = Aabb(Vec3f(-0.5f, 500f, -5.5f), Vec3f(0.5f, 501f, -4.5f))
    ...
    fun boxBelowTheBottomPlaneDoesNotIntersect() {
        val camera = identityViewCamera()
        val box = Aabb(Vec3f(-0.5f, -501f, -5.5f), Vec3f(0.5f, -500f, -4.5f))
    ...
    fun boxPastTheFarPlaneDoesNotIntersect() {
        val camera = identityViewCamera()
        val box = Aabb(Vec3f(-0.5f, -0.5f, -1000f), Vec3f(0.5f, 0.5f, -999f))
    ...
    fun boxInFrontOfTheNearPlaneDoesNotIntersect() {
        ...
        val box = Aabb(Vec3f(-0.5f, -0.5f, -0.5f), Vec3f(0.5f, 0.5f, 0f))
    ...
    fun containsSphereIsTrueForAPointOnTheForwardAxis() {
        ...
        assertTrue(planes.containsSphere(Vec3f(0f, 0f, -5f), radius = 0.1f))
    }

    @Test
    fun containsSphereIsFalseForAPointWellOutsideEveryPlane() {
        val planes = Frustum.planes(identityViewCamera(), aspect = 1f)
        assertTrue(!planes.containsSphere(Vec3f(0f, 500f, -5f), radius = 0.1f))
    }
    ...
    fun containsSphereRadiusRescuesAPointStraddlingAPlane() {
        ...
        assertTrue(!planes.containsSphere(Vec3f(0f, 0f, -0.9f), radius = 0.01f))
        assertTrue(planes.containsSphere(Vec3f(0f, 0f, -0.9f), radius = 0.5f))
    }

    private fun assertVec3Equals(
        expected: io.github.ronjunevaldoz.awake.core.math.Vec3f,
        actual: io.github.ronjunevaldoz.awake.core.math.Vec3f,
        epsilon: Float = 1e-4f
    ) {
        assertTrue(
