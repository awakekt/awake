...
class CameraTest {
     ...
     * exactly `projection`, so this is the one setup where `Lens`'s output can be
    ...
    fun `viewProjectionMatrix matches raw perspective matrix when view is identity`() {
        ...
        assertEquals(Lens.Projection.Perspective, camera.projection)
    ...
    fun `viewProjectionMatrix widens m00 as aspect ratio narrows`() {
        ...
        assertTrue(
            narrow.m00 > wide.m00,
            "expected narrow.m00 (${narrow.m00}) > wide.m00 (${wide.m00})"
        )
    }
    ...
    fun `a point above the centre lands on opposite NDC Y sides depending on the renderer's ClipSpace`() {
        val webGpuStyle =
            identityViewCamera().viewProjectionMatrix(aspect = 1f, clipSpace = ClipSpace.WebGpu)
        val vulkanStyle =
            identityViewCamera().viewProjectionMatrix(aspect = 1f, clipSpace = ClipSpace.Vulkan)
    ...
    fun `orthographic projects the same XY at any depth where perspective shrinks`() {
        ...
        val perspective =
            identityViewCamera().viewProjectionMatrix(aspect = 2f, clipSpace = ClipSpace.WebGpu)
        assertTrue(
