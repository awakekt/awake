...
class Mat4LookAtTest {
    ...
    fun axisAlignedCameraPutsTheLookAtTargetStraightAheadDownMinusZ() {
        val view =
            Mat4.setLookAt(eye = Vec3f(0f, 0f, 5f), center = Vec3f(0f, 0f, 0f), up = Vec3f.UP)
    ...
    fun theTranslationColumnIsMinusTheBasisProjectedOntoTheEye() {
        val eye = Vec3f(3f, 4f, 5f)
        val view = Mat4.setLookAt(eye = eye, center = Vec3f(0f, 0f, 0f), up = Vec3f.UP)

        val forward = (Vec3f(0f, 0f, 0f) - eye).normalized()
        val side = forward.cross(Vec3f.UP).normalized()
    ...
    fun viewMatrixShouldMapTheCameraPositionToTheOrigin() {
        for (eye in listOf(Vec3f(5f, 0f, 0f), Vec3f(3f, 4f, 5f), Vec3f(-2f, 7f, -1.5f))) {
            val view = Mat4.setLookAt(eye = eye, center = Vec3f(0f, 0f, 0f), up = Vec3f.UP)
    ...
    fun viewMatrixShouldHoldTheSideVectorInItsFirstRow() {
        val eye = Vec3f(5f, 0f, 0f)
        val view = Mat4.setLookAt(eye = eye, center = Vec3f(0f, 0f, 0f), up = Vec3f.UP)
        val side = Vec3f(-1f, 0f, 0f).cross(Vec3f.UP).normalized()
    ...
    fun viewMatrixShouldPutGeometryOnTheCamerasRightAtPositiveViewX() {
        val view =
            Mat4.setLookAt(eye = Vec3f(5f, 0f, 0f), center = Vec3f(0f, 0f, 0f), up = Vec3f.UP)
    ...
    /**
     * Forward parallel to up: `f.cross(up)` is the zero vector, and [Vec3f.normalize]'s zero guard
    ...
    fun forwardParallelToUpProducesADegenerateMatrixRatherThanNaNs() {
        val view =
            Mat4.setLookAt(eye = Vec3f(0f, 0f, 0f), center = Vec3f(0f, 1f, 0f), up = Vec3f.UP)
    ...
    fun aNearlyParallelUpVectorStillProducesAUsableMatrix() {
        val view = Mat4.setLookAt(
            ...
            up = Vec3f(0.001f, 1f, 0f),
        )

        assertTrue(view.data.none { it.isNaN() }, "expected no NaNs, got ${view.data.toList()}")
        // The side vector recovers to a unit vector even from a nearly-degenerate cross product.
        assertEquals(1f, Vec3f(view.m00, view.m01, view.m02).length3(), TOLERANCE)
    }

    @Test
    fun zeroLengthForwardIsDegenerateButStillNaNFree() {
        val view =
            Mat4.setLookAt(eye = Vec3f(1f, 2f, 3f), center = Vec3f(1f, 2f, 3f), up = Vec3f.UP)
    ...
    fun theNineComponentOverloadDelegatesToTheVectorOverload() {
        val fromVectors = Mat4.setLookAt(Vec3f(1f, 2f, 3f), Vec3f(4f, 5f, 6f), Vec3f(0f, 1f, 0f))
