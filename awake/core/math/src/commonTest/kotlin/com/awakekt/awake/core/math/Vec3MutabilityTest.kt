...
/**
 * Locks in [Vec3f]'s naming contract: a bare imperative verb mutates the receiver, the `-ed`
...
class Vec3MutabilityTest {
    ...
    fun normalizingAZeroVectorLeavesItAloneRatherThanProducingNaN() {
        val zero = Vec3f(0f, 0f, 0f).normalize()
    ...
    fun mutatingHelpersChainAndReturnTheReceiver() {
        val v = Vec3f(1f, 1f, 1f)
        val returned = v.set(2f, 4f, 6f).add(Vec3f(1f, 1f, 1f)).sub(Vec3f(1f, 1f, 1f)).scale(0.5f)
    ...
    fun upIsAFreshInstanceSoCallersCannotCorruptWorldUp() {
        ...
        assertEquals(1f, Vec3f.UP.y, TOLERANCE)
        assertEquals(-1f, Vec3f.FORWARD.z, TOLERANCE)
        assertEquals(1f, Vec3f.RIGHT.x, TOLERANCE)
    }
