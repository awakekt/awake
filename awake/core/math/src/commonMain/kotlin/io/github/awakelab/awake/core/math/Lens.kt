 ...
 *
 * A `Lens` describes a **lens** -- eye, target, up, field of view, near/far -- and nothing
 * else. It deliberately stores no clip-space convention: the active
 * [io.github.awakelab.awake.render.renderer.Renderer] owns that (it owns the API), and
 ...
 */
class Lens(
    var eye: io.github.awakelab.awake.core.math.Vec3f,
    var center: io.github.awakelab.awake.core.math.Vec3f,
    var up: io.github.awakelab.awake.core.math.Vec3f = Vec3f(0f, 1f, 0f),
    ...
    companion object {
         ...
         */
        fun perspective(
            eye: io.github.awakelab.awake.core.math.Vec3f = Vec3f(
                0f,
                0f,
                DEFAULT_EYE_DISTANCE,
            ),
            center: io.github.awakelab.awake.core.math.Vec3f = Vec3f.ZERO,
            up: io.github.awakelab.awake.core.math.Vec3f = Vec3f.UP,
        ...
        val DEFAULT_ORTHO_HALF_HEIGHT =
            DEFAULT_EYE_DISTANCE * tan(DEFAULT_FOV_DEGREES.angleRad / 2f)
...
fun Lens.projectToViewport(
    world: io.github.awakelab.awake.core.math.Vec3f,
    viewProjection: Mat4,
    viewportWidth: Float,
...
private fun Lens.isInFront(world: io.github.awakelab.awake.core.math.Vec3f): Boolean {
    ...
    val depth =
        ((world.x - eye.x) * forwardX + (world.y - eye.y) * forwardY + (world.z - eye.z) * forwardZ) /
            forwardLength
    return depth > near
}
...
fun Lens.rayThroughViewport(
    ...
    return if (dx * dx + dy * dy + dz * dz < DEGENERATE_LENGTH) {
        null
    } else {
        Ray.through(
            nearPoint,
...
private fun Mat4.unprojectNdc(
    ndcX: Float,
    ndcY: Float,
    ndcZ: Float,
): io.github.awakelab.awake.core.math.Vec3f? {
