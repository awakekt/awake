...
class MeshSimplifierTest {
    ...
    @Test
    fun lockedBoundariesPreservesBoundaryPositionsExactly() {
        val positions = FloatArray(9 * 3)
        var v = 0
        for (y in 0..2) {
            for (x in 0..2) {
        ...
        // All 8 boundary vertices (everything except center (1, 1)) must retain their exact position
        for (y in 0..2) {
            for (x in 0..2) {
                if (x == 1 && y == 1) continue // center interior vertex
                val origIdx = index(x, y)
                val newIdx = result.vertexRemap[origIdx]
                assertEquals(x.toFloat(), result.positions[newIdx * 3], 1e-4f)
                assertEquals(0f, result.positions[newIdx * 3 + 1], 1e-4f)
                assertEquals(y.toFloat(), result.positions[newIdx * 3 + 2], 1e-4f)
            }
        }
    }
