...
import io.github.awakelab.awake.asset.gltf.GltfParser.decodeBuffer
import io.github.awakelab.awake.asset.gltf.GltfParser.parse
import io.github.awakelab.awake.asset.gltf.GltfParser.parseScene
import io.github.awakelab.awake.asset.gltf.GltfParser.parseSkinned
import io.github.awakelab.awake.asset.gltf.GltfParser.readMaterialImageBytes
...
object GltfParser {
    ...
    /**
     * Extracts every external URI from a glTF JSON string that needs to be pre-fetched
     * (e.g. via `readResourceBytes`, relative to the `.gltf`'s own resource path) and pass
     * back in as `externalResources` before calling [parse]/[parseScene]/[parseSkinned].
     *
     * @param json The glTF JSON content.
     * @return List of external URIs.
     */
     ...
     *
     * @param bytes The GLB file bytes.
     * @param externalResources Map of external URIs to their pre-fetched byte content.
     */
    fun parseScene(
        bytes: ByteArray,
    ...
    fun parseSkinned(
        ...
        val skinnedNodes = document.nodes.mapIndexedNotNull { index, node ->
            ...
            if (meshIndex != null && skinIndex != null) {
                SkinnedNodeRef(
                    index,
                    meshIndex,
                    skinIndex,
     ...
     * source byte width, matching [io.github.awakelab.awake.core.geometry.VertexAttributeFormat.UInt4]'s
