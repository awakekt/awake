...
struct Uniforms {
  ...
  lightColor : vec4f,
  // Fixed slot arrays, not a variable-length buffer: this is a single uniform bind and neither
  // backend has a storage-buffer path here. xyz = world position, w = range; w <= 0 means the
  // slot is off, which is how a scene with fewer lights than slots costs only the loop.
...
fn fragmentMain(
  ...
  var lit = ambient + diffuse * radiance + specularOut / (specularOut + vec3f(1.0)) + emissive;

  // Point lights run the same BRDF with a per-light direction and an attenuation term. Unrolled
  // over a fixed slot count rather than a dynamic loop bound -- WGSL wants a constant here, and
  // a disabled slot is one compare, not a branch the whole wave pays for.
  for (var i = 0u; i < 4u; i = i + 1u) {
    let slot = uniforms.pointLightPositions[i];
    if (slot.w <= 0.0) { continue; }
    let toLight = slot.xyz - worldPos;
    let dist = length(toLight);
    if (dist >= slot.w) { continue; }
    let pl = toLight / max(dist, EPSILON);
    let pNdotL = max(dot(n, pl), 0.0);
    if (pNdotL <= 0.0) { continue; }
    // Inverse-square, windowed so the contribution reaches exactly zero at `range` instead of
    // asymptotically. Without the window a light never leaves the slot it occupies.
    let falloff = clamp(1.0 - (dist * dist) / (slot.w * slot.w), 0.0, 1.0);
    let attenuation = falloff * falloff / max(dist * dist, EPSILON);
    let ph = normalize(v + pl);
    let pNdotH = max(dot(n, ph), 0.0);
    let pFresnel = fresnelSchlick(max(dot(ph, v), 0.0), f0);
    let pSpecular = (distributionGgx(pNdotH, roughness) * geometrySmith(nDotV, pNdotL, roughness) *
      pFresnel) / max(4.0 * nDotV * pNdotL, EPSILON);
    let pDiffuse = (vec3f(1.0) - pFresnel) * (1.0 - metallic) * albedo / PI;
    let pRadiance = uniforms.pointLightColors[i].xyz * PI * pNdotL * attenuation;
    let pSpecularOut = pSpecular * pRadiance;
    lit = lit + pDiffuse * pRadiance + pSpecularOut / (pSpecularOut + vec3f(1.0));
  }
  return vec4f(
