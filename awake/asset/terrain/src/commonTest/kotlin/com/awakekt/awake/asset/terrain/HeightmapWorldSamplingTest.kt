...
class HeightmapWorldSamplingTest {
     ...
     *
     * The map is centred, so with unit scale its four samples sit at +/-0.5 rather than at 0..1:
     *
     *     z=+0.5:  2 --- 4
     *     z=-0.5:  0 --- 1
     */
    ...
    fun returnsTheExactSampleAtGridPoints() {
        ...
        assertEquals(0f, heightmap.heightAtWorld(-0.5f, -0.5f))
        assertEquals(1f, heightmap.heightAtWorld(0.5f, -0.5f))
        assertEquals(2f, heightmap.heightAtWorld(-0.5f, 0.5f))
        assertEquals(4f, heightmap.heightAtWorld(0.5f, 0.5f))
    }
    ...
    fun interpolatesAlongEachAxisIndependently() {
        ...
        assertEquals(0.5f, heightmap.heightAtWorld(0f, -0.5f), "Halfway between 0 and 1 in X.")
        assertEquals(1f, heightmap.heightAtWorld(-0.5f, 0f), "Halfway between 0 and 2 in Z.")
    }

    @Test
    fun blendsAllFourSamplesInsideACell() {
        // (0 + 1 + 2 + 4) / 4 at the cell centre, which for a centred map is the origin.
        assertEquals(1.75f, corners().heightAtWorld(0f, 0f))
    }
    ...
    fun appliesTheVerticalScaleUnlikeHeightAt() {
        ...
        assertEquals(40f, heightmap.heightAtWorld(0.5f, 0.5f), "heightAtWorld returns a world Y.")
    }

    @Test
    fun mapsWorldSpaceThroughTheHorizontalScale() {
        // 4 metres per sample in X, 8 in Z, and centred -- so sample (1, 1) sits at world (2, _, 4).
        val heightmap = corners(Vec3f(4f, 1f, 8f))

        assertEquals(1f, heightmap.heightAtWorld(2f, -4f))
        assertEquals(2f, heightmap.heightAtWorld(-2f, 4f))
        assertEquals(0.5f, heightmap.heightAtWorld(0f, -4f), "Halfway across one 4m cell.")
    }
    ...
    fun returnsNaNOutsideEveryEdge() {
        ...
        assertTrue(heightmap.heightAtWorld(-0.51f, 0f).isNaN(), "Past the -X edge.")
        assertTrue(heightmap.heightAtWorld(0.51f, 0f).isNaN(), "Past the +X edge.")
        assertTrue(heightmap.heightAtWorld(0f, -0.51f).isNaN(), "Past the -Z edge.")
        assertTrue(heightmap.heightAtWorld(0f, 0.51f).isNaN(), "Past the +Z edge.")
    }
    ...
    fun samplesExactlyOnTheFarEdge() {
        ...
        assertEquals(2f, heightmap.heightAtWorld(1f, -0.5f))
        assertEquals(5f, heightmap.heightAtWorld(1f, 0.5f))
    }
