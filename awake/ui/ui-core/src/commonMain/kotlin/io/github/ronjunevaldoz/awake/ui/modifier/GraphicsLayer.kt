 ...
 *
 * `ui-core` only owns this contract and does not know how to draw any effect. A concrete effect
 * is either a zero-property marker a widget interprets during its own emission, or one that
 * carries a value the emission choke point applies centrally (see [UiAlphaEffect]). This must not
 * be confused with `Canvas`/`CanvasScope`, which is an app-level escape hatch, not a
 * base-component implementation path.
 */
...
/**
 * Real alpha-compositing effect -- unlike a zero-property marker a widget interprets itself,
 * this actually carries the value a draw pass needs. Applied once, centrally,
 ...
 * needs to know this effect exists. This is a CPU-side per-primitive alpha
 ...
 * the backend as [io.github.ronjunevaldoz.awake.core.graphics2d.UiPrimitiveTransform] on the primitive
