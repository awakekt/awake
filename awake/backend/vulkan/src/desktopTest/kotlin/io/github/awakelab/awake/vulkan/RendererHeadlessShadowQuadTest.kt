...
class RendererHeadlessShadowQuadTest {
    ...
    private fun ByteArray.blueAt(x: Int, y: Int): Int = this[(y * TARGET + x) * 4 + 2].toInt() and 0xFF

    private fun render(renderer: io.github.awakelab.awake.vulkan.renderer.Renderer, shadow: UiDrawPrimitive.ShadowQuad): ByteArray {
    ...
    @Test
    fun linearGradientShadowReachesTheNativeRoundedShadowShader() {
        RendererHeadlessUiGlyphBaselineTest.withSharedHeadlessRenderer { renderer ->
            val red = Color(1f, 0f, 0f, 1f)
            val blue = Color(0f, 0f, 1f, 1f)
            val data = render(
                renderer,
                shadow(blurRadius = 2f).copy(
                    gradient = UiLinearGradient.horizontal(red, blue),
                ),
            )

            // Both samples are well inside the caster. A uniform-colour fallback would make one
            // of these channel comparisons fail even though the shadow still rendered.
            assertTrue(data.redAt(25, 32) > data.blueAt(25, 32), "left interior was not red")
            assertTrue(data.blueAt(39, 32) > data.redAt(39, 32), "right interior was not blue")
        }
    }
}
