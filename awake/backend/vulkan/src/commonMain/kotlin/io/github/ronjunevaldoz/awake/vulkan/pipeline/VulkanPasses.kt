...
/** Vulkan's [SkyboxPass]. */
internal class VulkanSkyboxPass(
    private val pipeline: SkyboxRenderPipeline,
) : SkyboxPass {
    override fun writeUniforms(frameIndex: Int, uniforms: FloatArray) =
        pipeline.writeUniforms(frameIndex, uniforms)

    override fun pipeline(frameIndex: Int): PipelineHandle = pipeline

    override fun uniformBinding(frameIndex: Int): MaterialBinding = pipeline.uniformBinding(frameIndex)

    /** The pipeline is this pass's to own -- it was constructor-injected into `Renderer` and
     * destroyed by `VulkanEngine` before the feature refactor. */
...
private class VulkanUiRunRecorder(
    ...
    override fun bindPipeline(kind: UiPipelineKind): Boolean {
        val pipeline = when (kind) {
            ...
            UiPipelineKind.Texture -> pipelines.texture
        }
    ...
    override fun drawTexturePrimitive(primitive: TexturedPrimitiveRun, slotIndex: Int) {
        val pipeline = pipelines.texture ?: return
