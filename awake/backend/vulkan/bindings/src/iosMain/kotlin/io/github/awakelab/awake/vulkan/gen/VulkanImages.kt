    ...
    actual fun vkTransitionImageLayout(
        ...
        val barrier = alloc<NativeVkImageMemoryBarrier>().apply {
            ...
            subresourceRange.apply {
                ...
                baseArrayLayer = 0u
                // Every layer, not just 0. A 2D-array image transitioned with layerCount = 1
                // leaves its upper layers in UNDEFINED, and sampling those returns garbage
                // without any validation error to say why.
                this.layerCount = layerCount.toUInt()
            }
        }
        val srcStage: UInt
        val dstStage: UInt
        if (oldLayout == VkImageLayout2.VK_IMAGE_LAYOUT_UNDEFINED &&
            ...
            newLayout == VkImageLayout2.VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL
        ) {
            // Reused offscreen targets must leave their sampled resting state before a new render pass.
            barrier.srcAccessMask = VK_ACCESS_SHADER_READ_BIT.toUInt()
            barrier.dstAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT.toUInt()
            srcStage = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT.toUInt()
            dstStage = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT.toUInt()
        } else if (oldLayout == VkImageLayout2.VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL &&
