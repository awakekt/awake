...
import io.github.awakelab.awake.vulkan.enums.flags.VkDebugUtilsMessageSeverityFlagBitsEXT
...
import io.github.awakelab.awake.vulkan.models.info.debug.PFN_vkDebugUtilsMessengerCallbackEXT
...
@OptIn(ExperimentalForeignApi::class)
private fun throwOnVkFailure(result: Int, operation: String) {
    if (result == VK_SUCCESS) return
    val vkResult = VkResult.values().firstOrNull { it.value == result }
        ?: error("$operation returned an unknown VkResult: $result")
    throw VkResultException("$operation failed: $result", vkResult)
}
...
actual object Vulkan {
    ...
    actual fun vkAcquireNextImageKHR(
        ...
        val result = nativeVkAcquireNextImageKHR(
            device.toCPointer(),
    ...
    actual fun vkQueuePresentKHR(queue: Long, pPresentInfoKHR: VkPresentInfoKHR) = memScoped {
        ...
        throwOnVkFailure(result, "vkQueuePresentKHR")
        Unit
    }
