...
inline jobject make_VkSharingMode(JNIEnv* env, const JNI_VkSharingMode& val) {
    jclass cls = env->FindClass("io/github/awakelab/awake/vulkan/enums/VkSharingMode");
...
inline JNI_VkBufferCreateInfo extract_VkBufferCreateInfo(JNIEnv* env, jobject obj) {
    ...
    jfieldID fid_sharingMode = env->GetFieldID(cls, "sharingMode", "Lio/github/awakelab/awake/vulkan/enums/VkSharingMode;");
    env->DeleteLocalRef(cls);
...
inline jobject make_VkBufferCreateInfo(JNIEnv* env, const JNI_VkBufferCreateInfo& val) {
    jclass cls = env->FindClass("io/github/awakelab/awake/vulkan/models/info/VkBufferCreateInfo");
    if (!cls) return nullptr;
    jmethodID ctor = env->GetMethodID(cls, "<init>", "(JIILio/github/awakelab/awake/vulkan/enums/VkSharingMode;)V");
    if (!ctor) { env->DeleteLocalRef(cls); return nullptr; }
    jobject _jenum_sharingMode = enum_from_ordinal(env, "io/github/awakelab/awake/vulkan/enums/VkSharingMode", val.sharingMode);
...
inline jobject make_VkMemoryAllocateInfo(JNIEnv* env, const JNI_VkMemoryAllocateInfo& val) {
    jclass cls = env->FindClass("io/github/awakelab/awake/vulkan/models/info/VkMemoryAllocateInfo");
    if (!cls) return nullptr;
...
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_vkCreateBuffer(
        JNIEnv* env,
        jclass clazz,
        jlong device,
        jobject createInfo) {
...
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_vkDestroyBuffer(
        JNIEnv* env,
        jclass clazz,
        jlong device,
        jlong buffer) {
...
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_vkGetBufferMemoryRequirements(
        JNIEnv* env,
        jclass clazz,
        jlong device,
        jlong buffer) {
...
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_findMemoryType(
        JNIEnv* env,
        jclass clazz,
...
extern "C" JNIEXPORT jlong JNICALL
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_vkAllocateMemory(
        JNIEnv* env,
        jclass clazz,
        jlong device,
...
extern "C" JNIEXPORT void JNICALL
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_vkFreeMemory(
        JNIEnv* env,
        jclass clazz,
        jlong device,
        jlong memory) {
...
extern "C" JNIEXPORT void JNICALL
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_vkBindBufferMemory(
        JNIEnv* env,
        jclass clazz,
        jlong device,
        jlong buffer,
...
extern "C" JNIEXPORT void JNICALL
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_writeBufferMemoryFloats(
        JNIEnv* env,
        jclass clazz,
        jlong device,
        jlong memory,
    ...
    // PERFORMANCE-CRITICAL -- do not reintroduce extract_float_array here.
    //
    // This is the hottest JNI entry point in the engine: every buffer write in the backend goes
    // through it or its byte sibling, and Studio alone issues 144 a frame -- vertices and indices
    // for each of ~72 draw runs -- carrying about 2 MB. extract_float_array allocates a
    // std::vector and copies the whole array into it, leaving the memcpy below to copy it a
    // second time into mapped memory: an allocation and a full extra copy, per call. Measured on
    // a real device, that cost 8.46 ms per drawUi against 1.18 ms for the form below, on a frame
    // shaped like Studio's. It was most of a 40 ms frame.
    //
    // GetFloatArrayRegion writes the JVM array straight into the mapped pointer: one copy, no
    // allocation. UiUploadCostBenchmark measures this path; StudioFrameCostRatchetTest guards the
    // geometry volume that feeds it. Change either at your peril, and re-measure if you do.
    ...
    // vkMapMemory rejects a zero size, and an empty write has nothing to do anyway.
    if (count == 0) return;

    void* mapped = nullptr;
    VkDeviceSize sizeBytes = static_cast<VkDeviceSize>(count) * sizeof(float);
    ...
    env->GetFloatArrayRegion(data, 0, count, reinterpret_cast<jfloat*>(mapped));
    vkUnmapMemory(reinterpret_cast<VkDevice>(device_ptr), reinterpret_cast<VkDeviceMemory>(memory_ptr));
}


extern "C" JNIEXPORT void JNICALL
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_writeBufferMemoryBytes(
        JNIEnv* env,
        jclass clazz,
        jlong device,
        jlong memory,
    ...
    // offset is a plain byte offset, not a handle -- 0 is valid and common (see the
    // memoryOffset note in vkBindBufferMemory above).
    // PERFORMANCE-CRITICAL -- straight into mapped memory, and do not reintroduce
    // extract_byte_array. See writeBufferMemoryFloats above for the measurement.
    const jsize count = env->GetArrayLength(data);
    if (env->ExceptionCheck()) return;
    ...
    void* mapped = nullptr;
    VkDeviceSize sizeBytes = static_cast<VkDeviceSize>(count);
    VkResult result = vkMapMemory(
        reinterpret_cast<VkDevice>(device_ptr),
        reinterpret_cast<VkDeviceMemory>(memory_ptr),
        static_cast<VkDeviceSize>(offset),
    ...
    env->GetByteArrayRegion(data, 0, count, reinterpret_cast<jbyte*>(mapped));
    vkUnmapMemory(reinterpret_cast<VkDevice>(device_ptr), reinterpret_cast<VkDeviceMemory>(memory_ptr));
}


extern "C" JNIEXPORT void JNICALL
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_vkCmdBindVertexBuffers(
        JNIEnv* env,
        jclass clazz,
        jlong commandBuffer,
        jint firstBinding,
...
extern "C" JNIEXPORT void JNICALL
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_vkCmdBindIndexBuffer(
        JNIEnv* env,
        jclass clazz,
        jlong commandBuffer,
        jlong buffer,
...
extern "C" JNIEXPORT void JNICALL
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_vkCmdCopyBuffer(
        JNIEnv* env,
        jclass clazz,
        jlong commandBuffer,
        jlong srcBuffer,
...
extern "C" JNIEXPORT void JNICALL
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_vkCmdDrawIndexed(
        JNIEnv* env,
        jclass clazz,
        jlong commandBuffer,
        jint indexCount,
...
extern "C" JNIEXPORT void JNICALL
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_vkDeviceWaitIdle(
        JNIEnv* env,
        jclass clazz,
        jlong device) {
    // --- Marshalling ---
...
Java_io_github_awakelab_awake_vulkan_gen_VulkanBuffers_readBufferMemoryBytes(
        JNIEnv* env,
        jclass clazz,
        jlong device,
        jlong memory,
