...
private class MutableFlexItemConfig : FlexItemConfig {
    ...
    override fun grow(value: Float) {
        require(value >= 0f) { "grow must be >= 0, was $value" }
        grow = value
    }
    override fun shrink(value: Float) {
        require(value >= 0f) { "shrink must be >= 0, was $value" }
        shrink = value
    }
...
private class FlexItemNode(var data: FlexItemData) :
    Modifier.Node(),
...
private class FlexBoxMeasurePolicy(private val config: FlexBoxConfig) : MeasurePolicy {
        ...
        return layout(width, height) {
            ...
            for (lineIndex in lines.indices) {
                ...
                for (child in line) {
                    ...
                    if (vertical) {
                        placeable.placeRelativeAt(crossCursor + crossOffset, placedMain)
                    } else {
                        placeable.placeRelativeAt(placedMain, crossCursor + crossOffset)
                    }
    ...
    private fun resolveMainSizes(line: List<FlexChild>, limit: Int, gap: Int, vertical: Boolean) {
        ...
        val totalFactor = if (delta >= 0) {
            line.sumOf { it.item.grow.toDouble() }.toFloat()
        } else {
            line.sumOf { (it.item.shrink * it.main).toDouble() }.toFloat()
        }
    ...
    override fun Density.minIntrinsicWidth(measurables: List<com.awakekt.awake.compose.ui.layout.IntrinsicMeasurable>, height: Int): Int =
        measurables.sumOf { it.minIntrinsicWidth(height) }

    override fun Density.maxIntrinsicWidth(measurables: List<com.awakekt.awake.compose.ui.layout.IntrinsicMeasurable>, height: Int): Int =
