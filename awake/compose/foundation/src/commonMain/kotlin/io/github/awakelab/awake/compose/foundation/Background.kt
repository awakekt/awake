...
private class BackgroundNode : Modifier.Node(), DrawModifierNode {
...
private class BorderElement(
    ...
    override fun update(node: BorderNode) {
        node.strokeWidth = strokeWidth; node.color = color; node.shape = shape; node.sides = sides
    }
    override fun toString(): String = "border($strokeWidth, $color, shape=$shape, sides=$sides)"
}

private class BorderNode : Modifier.Node(), DrawModifierNode {
...
private fun borderOutlinePath(
    ...
    return when (outline) {
        is ShapeOutline.Rectangle -> io.github.awakelab.awake.core.graphics2d.DrawShape.Rectangle.toPath(bounds)
        is ShapeOutline.Rounded -> io.github.awakelab.awake.core.graphics2d.DrawShape.RoundedRectangle(outline.radius.dp)
            .toPath(bounds)
