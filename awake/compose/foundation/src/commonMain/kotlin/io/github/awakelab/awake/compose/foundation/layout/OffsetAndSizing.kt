...
private class OffsetNode : Modifier.Node(), LayoutModifierNode {
    var x: Dp = 0.dp
    var y: Dp = 0.dp
...
private class AbsoluteOffsetElement(
    ...
    override fun update(node: AbsoluteOffsetNode) { node.x = x; node.y = y }
}

private class AbsoluteOffsetNode : Modifier.Node(), LayoutModifierNode {
    var x: Dp = 0.dp
    var y: Dp = 0.dp
    override fun MeasureScope.measure(measurable: Measurable, constraints: Constraints): MeasureResult {
        val placeable = measurable.measure(constraints)
...
private class AspectRatioElement(
    ...
    override fun update(node: AspectRatioNode) { node.ratio = ratio }
}

private class AspectRatioNode : Modifier.Node(), LayoutModifierNode {
    var ratio: Float = 1f
    override fun MeasureScope.measure(measurable: Measurable, constraints: Constraints): MeasureResult {
...
private class SizeInElement(
    ...
    override fun update(node: SizeInNode) {
        node.minWidth = minWidth; node.maxWidth = maxWidth
        node.minHeight = minHeight; node.maxHeight = maxHeight
    }
}

private class SizeInNode : Modifier.Node(), LayoutModifierNode {
...
private class RequiredSizeElement(
    ...
    override fun update(node: RequiredSizeNode) { node.width = width; node.height = height }
}

private class RequiredSizeNode : Modifier.Node(), LayoutModifierNode {
    var width: Dp? = null
    var height: Dp? = null
    override fun MeasureScope.measure(measurable: Measurable, constraints: Constraints): MeasureResult {
...
private class DefaultMinSizeElement(
    ...
    override fun update(node: DefaultMinSizeNode) { node.minWidth = minWidth; node.minHeight = minHeight }
}

private class DefaultMinSizeNode : Modifier.Node(), LayoutModifierNode {
    var minWidth: Dp = Dp.Unspecified
    var minHeight: Dp = Dp.Unspecified
    override fun MeasureScope.measure(measurable: Measurable, constraints: Constraints): MeasureResult {
