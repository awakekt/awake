...
package io.github.ronjunevaldoz.awake.compose.foundation.interaction
 ...
 * Coroutines are an explicit non-goal for this engine (see the README), so the flow collapses to
 * plain properties. `tryEmit` is already non-suspending in Compose, so only the read side differs.
 */
