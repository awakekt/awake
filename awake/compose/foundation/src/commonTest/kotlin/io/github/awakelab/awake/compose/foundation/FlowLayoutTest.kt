...
class FlowLayoutTest {
    ...
    private fun tagPositions(content: context(Composer) () -> Unit): Map<String, Pair<Int, Int>> {
        return taggedNodes(content).mapValues { (_, node) -> node.x to node.y }
    }
    ...
    private fun taggedNodes(content: context(Composer) () -> Unit): Map<String, SemanticsNode> {
        ...
        fun collect(values: List<SemanticsNode>) { values.forEach { nodes += it; collect(it.children) } }
        collect(frame.semantics)
        return nodes.filter { it.testTag != null }.associateBy { node -> node.testTag!! }
    }

    context(_: Composer)
    private fun io.github.awakelab.awake.compose.foundation.layout.RowScope.items() { repeat(3) { Box(Modifier.size(20.dp, 10.dp).testTag("item$it")) } }
    context(_: Composer)
    private fun io.github.awakelab.awake.compose.foundation.layout.ColumnScope.items() { repeat(3) { Box(Modifier.size(20.dp, 10.dp).testTag("item$it")) } }
}
