 ...
 */
class Composer internal constructor(private val applier: Applier, root: Any? = null) {
    ...
    internal val locals = ProvidedLocals()
     ...
     */
    internal fun <T> remember(key: Any?, calculate: () -> T): T {
        ...
        if (index == slots.size) slots.add(RememberedSlot(key, calculate()))
...
fun reconcile(
    ...
    val composer = Composer(applier, root)
    content(composer)
    composer.finishRoot()
    return composer
}
