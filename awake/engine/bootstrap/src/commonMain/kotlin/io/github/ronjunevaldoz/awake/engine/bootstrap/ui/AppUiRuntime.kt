...
import io.github.ronjunevaldoz.awake.engine.bootstrap.ui.AppUiRuntime.Companion.EMPTY_UI_ONLY_CAMERA
import io.github.ronjunevaldoz.awake.engine.bootstrap.ui.AppUiRuntime.Companion.FRAME_TIME_HISTORY_SIZE
...
class AppUiRuntime(
    ...
    /** Public (not private-set) so an installing [io.github.ronjunevaldoz.awake.engine.platform.core.AppModule] -- see [provideDrawCalls]'s doc
     ...
     * (a `AppUiRuntime.() -> Unit`, no delta parameter of its own) can still advance
    ...
    /** Optional per-frame hook an installing [io.github.ronjunevaldoz.awake.engine.platform.core.AppModule] sets (from its own `overlay`/
     * `onReady` block, which has `this: AppUiRuntime` access) to route real 3D geometry into
    ...
    var provideDrawCalls: (() -> Pair<Lens, List<DrawCall>>)? = null
    ...
    fun rootColumn(
        ...
        val frame = Rectangle(0f, 0f, viewportWidth, viewportHeight)
