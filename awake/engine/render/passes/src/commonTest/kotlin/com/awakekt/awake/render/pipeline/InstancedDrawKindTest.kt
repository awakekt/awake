...
class InstancedDrawKindTest {
    ...
    private class FakeGpuMesh(override val format: VertexFormat) : GpuMesh

    private class FakeGpuMaterial : GpuMaterial {
        override fun updateUniformBuffer(uniformFloats: FloatArray) = Unit
        override fun destroy() = Unit
    }
    ...
    @Test
    fun sourcePacketsUseTheSameClassificationAsResolvedDraws() {
        val source = RenderDrawCommand(
            mesh = FakeGpuMesh(VertexFormat.PositionUv),
            material = FakeGpuMaterial(),
            instanceModels = listOf(Mat4()),
        )

        assertEquals(InstancedDrawKind.Particle, source.instancedDrawKind())
    }

    @Test
    fun sourcePacketsShareDepthCasterIdentityAndAlphaMode() {
        val source = RenderDrawCommand(
            mesh = FakeGpuMesh(VertexFormat.PositionNormalColorSkin),
            material = FakeGpuMaterial(),
            instanceModels = listOf(Mat4()),
            instanceJointPalettes = listOf(FloatArray(16)),
            alphaMode = AlphaMode.Masked,
        )

        assertEquals(
            DepthRenderKey(DepthCasterKind.SkinnedInstanced, AlphaMode.Masked),
            source.depthRenderKey(),
        )
    }
