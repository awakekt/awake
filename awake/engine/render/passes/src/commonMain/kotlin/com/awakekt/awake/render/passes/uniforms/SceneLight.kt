 ...
 * (one multiply in the shader, not a separate intensity uniform). Backend-neutral; `RenderSystem3D`
 * builds one from the scene's `Light` entities, which can depend on this module where the reverse
 * would be a cycle.
 *
 * [points] is **borrowed for the frame, not owned**: `RenderSystem3D` refills one list each frame
...
data class SceneLight(
     ...
     * for the rule. `RenderSystem3D` fills this in.
     *
...
/** Number of cube faces in one positional-light shadow allocation. */
const val POINT_SHADOW_FACE_COUNT = 6

/** Total layer capacity for the shared directional-plus-positional shadow target. */
const val MAX_SHADOW_TARGET_LAYERS = MAX_SHADOW_CASCADES + MAX_POINT_LIGHTS * POINT_SHADOW_FACE_COUNT
