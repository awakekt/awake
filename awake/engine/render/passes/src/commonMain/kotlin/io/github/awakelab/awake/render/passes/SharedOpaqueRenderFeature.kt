...
class SharedOpaqueRenderFeature {
    ...
    fun recordDraws(recorder: CommandRecorder, draws: List<PreparedDraw>) {
        var lastVertexBuffer: io.github.awakelab.awake.render.command.BufferHandle? = null
        var lastIndexBuffer: io.github.awakelab.awake.render.command.BufferHandle? = null
        var index = 0
        while (index < draws.size) {
            ...
            recorder.bindMaterial(BindingSemantic.Material, draw.materialBinding)
            draw.instanceVertexBuffer?.let { recorder.bindVertexBuffer(INSTANCE_MODEL_BINDING, it) }
            draw.jointPaletteBinding?.let { recorder.bindMaterial(BindingSemantic.JointPalette, it) }
            draw.shadowBinding?.let { recorder.bindMaterial(BindingSemantic.ShadowDepth, it) }
            draw.sceneDepthBinding?.let { recorder.bindMaterial(BindingSemantic.SceneDepth, it) }
