...
class FontBaselineFidelityTest {
    ...
    private val knownBaselineDrift = mapOf(
        // Measured 2026-08-10 after two independent fixes landed: the atlas is now generated by
        // :awake:engine:ui:font-atlas-generator from TTF outline geometry (continuous) instead
        // of the antialiased raster ink bbox (quantized to 1/64 em), and BasicText now snaps a
        // line's pen origin once instead of rounding each glyph's own edges.
        //
        // Neither closed this. The count is 3, exactly as it was before both fixes -- only the
        // membership moved (roundvsflat-12 closed, roundvsflat-16 opened). That is the useful
        // result: the residual is NOT metric quantization and NOT glyph rounding, because it
        // survives the removal of both. Note roundvsflat-12 renders clean at the atlas's own
        // cellSize of 12, where texels map 1:1; email-12 does not, so scale alone is not the
        // whole story either.
        //
        // The remaining suspect is the coverage atlas itself -- resampling one fixed-cell
        // CoverageAlpha raster to arbitrary sizes cannot reproduce Chromium's per-size
        // rasterization. That is what MTSDF is for, and the MSDF shader path already exists
        // unused (see UiFonts.msdf, which has no callers). Left recorded, not force-closed.
        "roundvsflat-14" to 1,
        "roundvsflat-16" to 1,
        "email-12" to 1,
    )
