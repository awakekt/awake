...
class GameplayInput(
    ...
    /** False while the UI holds the pointer, so a drag over a panel is not a world drag. */
    val pointerDown: Boolean get() = snapshot.pointerDown && !claimed.isCaptured

    val pointerX: Float get() = snapshot.pointerX

    val pointerY: Float get() = snapshot.pointerY

    /** Zero while a scrollable under the pointer took the wheel. */
    val scrollDeltaY: Float get() = if (claimed.isScrollConsumed) 0f else snapshot.scrollDeltaY

    /**
     * Whether [button] is held for the world, gated by the same capture rule as [pointerDown].
     *
     ...
     */
    fun isDown(button: PointerButton): Boolean = !claimed.isCaptured && snapshot.isDown(button)
