...
private fun World.sceneComponents(
    ...
    // Named, because the behaviour exporters resolve an entity reference and so need the World
    // itself rather than the list being built.
    val world = this
    return buildList {
        ...
        get<PatrolBehavior>(entity)?.let { add(it.toSceneComponent()) }
        get<ChaseBehavior>(entity)?.let { add(it.toSceneComponent(world, entity)) }
        get<FleeBehavior>(entity)?.let { add(it.toSceneComponent(world, entity)) }
...
internal fun Vec3f.toSceneVec3(): SceneVec3 = SceneVec3(x, y, z)
