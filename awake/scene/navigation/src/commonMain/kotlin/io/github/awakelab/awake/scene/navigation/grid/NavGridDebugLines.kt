 ...
 */
private class NavGridDebugSink(private val surfaceAt: (Float, Float) -> Float) {
    ...
    private fun cross(worldX: Float, worldZ: Float, arm: Float, lift: Float, color: Color) {
    ...
    private fun Vec3f.onSurface() = Vec3f(x, surfaceAt(x, z) + ROUTE_LIFT, z)
}

/** Checkerboard by cell, so a tile drawn at the wrong offset breaks a pattern instead of hiding. */
private fun blockedColorFor(coord: WorldCellCoord): Color =
...
private fun routeColorFor(entity: Entity): Color =
...
private val BLOCKED_COLOR = Color(r = 0.95f, g = 0.25f, b = 0.2f)
private val BLOCKED_COLOR_ALTERNATE = Color(r = 0.85f, g = 0.45f, b = 0.15f)
