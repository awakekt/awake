...
class RenderSystem(
     ...
     * level verification, same reasoning [io.github.awakelab.awake.scene.rendering.components.WorldDebugSettings]'s
    ...
    private fun sceneLight(world: World): SceneLight {
        ...
        //
        // Allocates a matrix per frame, but only while shadows are on: `directionalShadowBox`
        // returns fresh matrices, and the caller that used to do this allocated the same ones.
        return if (renderer.shadowsEnabled) {
            base.copy(viewProjection = directionalShadowBox(base.direction, renderer.clipSpace).viewProjection)
        } else {
            base
        }
    }
