...
/**
 * Keeps the scene's [SpatialIndex] current, so [RenderSystem3D] can ask which entities are in
 * front of the camera instead of testing every one of them.
 *
 * Install it before [RenderSystem3D] -- what it writes this frame is what culling reads this
 * frame. A scene without it has no index and [RenderSystem3D] falls back to testing each entity,
