...
#
# INVESTIGATION: summarises component status.
...
Usage:
    python3 .agents/skills/awake-ui-verification/scripts/generate_ui_status.py
"""
...
# The skill is vendored under `.agents/skills/<skill>/scripts`.
REPO = Path(__file__).resolve().parents[4]
OUT = REPO / "docs/reference/ui-fidelity-status.md"
COMPOSE_UI = REPO / "awake/compose/ui/src"
FOUNDATION = REPO / "awake/compose/foundation/src"
GRAPHICS = REPO / "awake/core/graphics2d/src"
TEXT = REPO / "awake/core/text/src"
...
def build_rows() -> list[tuple[str, str, str, str, str]]:
    ...
    # --- Rendering -------------------------------------------------------
    scanline = grep_count(GRAPHICS, r"scanlineFillTriangulation")
    rows.append((
    ...
    fringe = grep_count(GRAPHICS, r"offsetPolygon")
    rows.append((
    ...
    adaptive = grep_count(GRAPHICS, r"CURVE_FLATNESS_TOLERANCE_PX")
    rows.append((
    ...
    stroke_join_reads = grep_count(GRAPHICS, r"stroke\.join|UiStrokeJoin\.")
    rows.append((
    ...
    # --- Assets ----------------------------------------------------------
    hand_written = grep_count(FOUNDATION, r"transcribed|ported by hand")
    cubics = grep_count(FOUNDATION, r"cubicTo\(")
    rows.append((
        "Icons", DONE if cubics and not hand_written else PARTIAL,
        f"{cubics} cubic segments, generated from SVG",
        "`skills/awake-ui-icons/scripts/svg_to_ui_image_vector.py --self-test`",
        "Fill-only: no outline tier until stroke joins exist.",
    ))

    advance_test = exists(
        "awake/core/text/src/commonTest/kotlin/io/github/awakelab/"
        "awake/core/text/font/PackedUiFontAdvanceTest.kt"
    )
    rows.append((
    ...
    rows.append((
        ...
        "theme-driven" if ui_shape_pins == 0 else f"{ui_shape_pins} pinned to a legacy global",
        "`Shadcn*FidelityTest`",
        "Seven non-default presets have no parity coverage.",
    ))

    ring = grep_count(FOUNDATION, r"\bringWidth\b|fun StyleScope\.ring\(")
    rows.append((
    ...
    hardcoded = grep_count(FOUNDATION, r"Dimension\.Fixed\(40f\.dp\)|40f\.dp")
    rows.append((
    ...
    # --- Behaviour -------------------------------------------------------
    key_sources = list((COMPOSE_UI / "commonMain").rglob("*.kt")) if (COMPOSE_UI / "commonMain").exists() else []
    keys = set(re.findall(r"Key\.(\w+)", "\n".join(path.read_text(errors="ignore") for path in key_sources)))
    rows.append((
    ...
    disabled = grep_count(FOUNDATION, r"alpha\(")
    rows.append((
    ...
    orphaned = [
        ...
        and not grep_count(FOUNDATION, rf"fun (?:\w+Scope|UiScope)(?:<[^>]+>)?\.{name}\b")
    ]
    rows.append((
        "Layer ownership", PARTIAL if orphaned else DONE,
        f"{len(orphaned)} branded component(s) with no Foundation primitive",
        "`verifyUiOwnership`",
...
def main() -> None:
    ...
    add("")
    add("**Generated by `.agents/skills/awake-ui-verification/scripts/generate_ui_status.py` — do not hand-edit.**")
    add(f"Reflects the working tree at `{git_head()}`.")
    add("")
    add("Where Awake's UI stands against its targets — shadcn/ui for the design system, Compose")
    add("Foundation-shaped behaviour, and correct rendering for the engine — and what is still missing.")
