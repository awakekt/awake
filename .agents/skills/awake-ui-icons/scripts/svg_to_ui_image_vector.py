...
imageVector(...) { path { ... } }` Kotlin block using only the commands
...
Usage:
    python3 skills/awake-ui-icons/scripts/svg_to_ui_image_vector.py icon.svg --name chevronDown \
...
_PACKED_CAPS = {"butt": "b", "round": "r", "square": "s"}
_PACKED_JOINS = {"miter": "m", "round": "r", "bevel": "b"}
_PACKED_OPERANDS = {"M": 2, "L": 2, "Q": 4, "C": 6, "Z": 0}


def num(v: float) -> str:
    """`fmt` without the Kotlin `f` suffix, for the packed string."""
    return fmt(v)[:-1]


def pack_commands(subpaths) -> str:
    """The command stream as path data: absolute M/L/Q/C/Z, with the letter dropped when it repeats
    the previous command (`L1 2 3 4` is two lines) and the separator dropped before a negative."""
    out = []
    previous = None
    for subpath in subpaths:
        for op, args in subpath:
            if op != previous:
                out.append(op)
                # Z takes no operands, so a following command always needs its own letter.
                previous = op if _PACKED_OPERANDS[op] else None
            for value in args:
                text = num(value)
                separated = out[-1][-1] not in "MLQCZ" and not text.startswith("-")
                out.append(" " + text if separated else text)
    return "".join(out)


def emit_packed(name, viewport, blocks, dp, source, indent="    ", level=2):
    """The `emit_kotlin` geometry as one string constant -- see `PackedImageVector.kt`."""
    i0 = indent * level
    i1 = indent * (level + 1)
    sections = [f"{num(dp)} {num(dp)} {num(viewport[0])} {num(viewport[1])}"]
    for subpaths, kind, extra in blocks:
        if kind == "stroke":
            width, cap, join = extra
            style = f"s{num(width)}{_PACKED_CAPS.get(cap, 'b')}{_PACKED_JOINS.get(join, 'm')}"
        else:
            style = "e" if extra == "EvenOdd" else "f"
        sections.append(f"{style}:{pack_commands(subpaths)}")
    lines = []
    if source:
        lines.append(f"{i0}/** {source}, generated by .agents/skills/awake-ui-icons/scripts/svg_to_ui_image_vector.py -- do not hand-edit. */")
    lines.append(f"{i0}val {name}: ImageVector = packedImageVector(")
    lines.append(f'{i1}"{"|".join(sections)}",')
    lines.append(f"{i0})")
    return "\n".join(lines)
...
def emit_kotlin(name, viewport, blocks, dp, source, indent="    ", level=2):
    ...
    if source:
        lines.append(f"{i0}/** {source}, generated by .agents/skills/awake-ui-icons/scripts/svg_to_ui_image_vector.py -- do not hand-edit. */")
    lines.append(f"{i0}val {name}: ImageVector = imageVector(")
    ...
    for subpaths, kind, extra in blocks:
        if kind == "stroke":
            ...
            header = (
                f"{i1}path(stroke = DrawStroke(width = {fmt(width)}.dp, "
...
def convert(svg_path, name, dp, source, indent_level, packed=False):
    ...
    emit = emit_packed if packed else emit_kotlin
    return emit(name, viewport, blocks, dp, source, level=indent_level)
...
def kdoc(lines, indent):
    """A `/** … */` block, or nothing when there is no prose to carry."""
    if not lines:
        return []
    if len(lines) == 1:
        return [f"{indent}/** {lines[0]} */"]
    out = [f"{indent}/**"]
    out += [f"{indent} *{(' ' + line) if line else ''}" for line in lines]
    out.append(f"{indent} */")
    return out
...
def emit_file(manifest, svg_root, args):
    ...
    for tier in manifest["tiers"]:
        ...
        for index, filename in enumerate(icons):
            stem = filename[:-4]
            source = f"{manifest['source']['name']} `{stem}` ({tier['label']})"
            if index:
                body.append("")
...
def camel_case(stem: str) -> str:
    """`arrow-down-tray` -> `arrowDownTray`. Heroicons and Lucide both name files in kebab-case and
    the Kotlin vals are the same words in camelCase, so batch mode derives one from the other
    instead of asking for 75 `--name` flags."""
    head, *rest = [part for part in re.split(r"[-_]", stem) if part]
    return head + "".join(part[:1].upper() + part[1:] for part in rest)


def self_test():
    # batch mode derives Kotlin val names from kebab-case filenames
    assert camel_case("chevron-down") == "chevronDown"
    assert camel_case("arrow-down-tray") == "arrowDownTray"
    assert camel_case("plus") == "plus"
    assert camel_case("square-2-stack") == "square2Stack"
...
def main():
    ...
    ap.add_argument(
        ...
        nargs="+",
        metavar="SVG",
        help="convert several SVGs in one run; val names come from the kebab-case filenames",
    )
    ap.add_argument(
        "--source-template",
        default="",
        help="per-icon provenance note for --batch, with {stem} replaced by the filename, "
        "e.g. 'Heroicons `{stem}` (24/outline)'",
    )
    ap.add_argument(
        "--packed",
        action="store_true",
        help="emit one packedImageVector(\"…\") string constant instead of a builder block",
    )
    ap.add_argument(
        "--manifest",
        help="emit a whole Kotlin file from a manifest.json describing the tiers beside the SVGs",
    )
    ...
    if args.manifest:
        ...
        sys.stdout.write(emit_file(manifest, os.path.dirname(args.manifest), args))
        return
    if args.batch:
        # A whole tier is one process, not one per icon: regenerating Heroicons used to be 75
        # interpreter starts, and every one of them re-imported and re-parsed this file.
        for index, svg in enumerate(sorted(args.batch)):
            stem = os.path.splitext(os.path.basename(svg))[0]
            source = args.source_template.format(stem=stem) if args.source_template else ""
            if index:
                print()
            print(emit_one(svg, camel_case(stem), args, source))
        return
    if not args.svg or not args.name:
        ap.error("svg and --name are required (or pass --batch)")

    print(emit_one(args.svg, args.name, args, args.source))


def emit_one(svg, name, args, source):
    processed_svg = preprocess_svg(svg, use_svgo=args.svgo, expand_strokes=args.expand_strokes)
    try:
        viewport, _ = parse_svg(processed_svg)
        dp = args.dp if args.dp is not None else viewport[0]
        return convert(processed_svg, name, dp, source, args.indent_level, packed=args.packed)
    finally:
        if processed_svg != svg and os.path.exists(processed_svg):
