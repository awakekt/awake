...
5. **Hardware Only**: A graphics backend knows pipelines, buffers, textures, samplers and command recording. It must never know what a skybox or a shadow *is*. A backend that declares `SkyboxRenderPipeline` has become a game, and a fourth content feature then costs an edit to both backends. Enforced by `verifyBackendLayering` on `check`; 5 files per backend sit in its tracked-debt ledger today and will reach 0 in Phase 2. Content belongs in the shared layer or `samples/<game>/` — see `docs/reference/render-extensibility.md` for the content-versus-capability test and `docs/tasks/2026-09-09-hal-vs-render-graph-phase-1-plan.md` for the exit. Opt-in is not absent: a nullable content-pipeline param still couples the backend to the content.
