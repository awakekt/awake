...
3. **Pure Hardware Abstraction (HAL vs Render Graph)**: A backend renderer receives pre-packed GPU passes (`GpuPassInput`, `GpuSubPass`, `GpuDrawCommand`) only and must have ZERO knowledge of scene lights, cameras (`Lens`), environment uniforms, fog, or shadow cascade math. Scene compilation belongs in `render:passes`.
...
5. **Hardware Only**: A graphics backend knows pipelines, buffers, textures, samplers and command recording. It must never know what a skybox or a shadow *is*. A backend that declares `SkyboxRenderPipeline` has become a game, and a fourth content feature then costs an edit to both backends. `verifyBackendLayering` runs on `check` against every backend production source; its import and content-vocabulary exemption ledgers are currently empty. Remaining backend cleanup is internal commonisation work tracked in the render handoff, not permission to add scene types. Content belongs in the shared layer or `samples/<game>/` — see `docs/reference/render-extensibility.md` for the content-versus-capability test and `docs/tasks/2026-09-09-hal-vs-render-graph-phase-1-plan.md` for the exit. Opt-in is not absent: a nullable content-pipeline param still couples the backend to the content.
...
9. **Uniform guard**: Every production uniform write must use the declared `UniformLayout` and
   `UniformWriter`; never infer a field from a numeric index, concatenate backend arrays, or
   detect WebGPU bindings by searching shader text. Run `./gradlew verifyRenderUniforms` with every
   backend or shader change. A new violation is a build failure, not an exemption request.
