...
That settles the consumer rule, but do not read it as `ui-headless` ≈ `compose-foundation`
(this skill previously did). Foundation stops below controls: it has no `Button`, `Checkbox`,
`Switch` or `Slider`, and Compose ships all of those in Material. `ui-headless` is
Foundation-shaped for its ~12 layout/text/draw primitives and Material-shaped for its ~21
controls with the styling stripped, whose real upstream is Radix — what shadcn is built on.
`ui-core` ≈ `compose-ui` still holds. Full rationale, including why this matters for the
